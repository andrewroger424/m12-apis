...
movie.as.data.frame <- as.data.frame(movie.data)
...
Director <- function(movie.df){
  return(paste0(movie.df$Title, ' was directed by ', movie.df$Director))
}

# Call Director with your favorite movie, and assign it to the variable movie.director
movie.director <- Director(movie.as.data.frame)
...
movie.lsit <- c('Saving Private Ryan', 'Braveheart', 'Finding Dory')
# Write a function to make your API request and process the data
MovieProcessing <- function(movies){
  query.parameters <- list(t = movie, r = 'json')
